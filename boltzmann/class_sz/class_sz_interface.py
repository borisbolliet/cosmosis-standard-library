...
def setup(options):
    # skip this version checks for now.
    # class_version = options.get_string(option_section, "version", "3.2.0")
    # pyversion = f"{sys.version_info.major}.{sys.version_info.minor}"
    # install_dir = dirname + f"/class_v{class_version}/classy_sz_install/lib/python{pyversion}/site-packages/"
    ...
    import classy_sz
    print(f"Loaded classy_sz from {classy_sz.__file__}")

    # Read options from the ini file which are fixed across
    # the length of the chain
    config = {
        'use_class_sz_fast_mode': options.get_int(option_section, 'use_class_sz_fast_mode', default=0),
        'ndim_redshifts': options.get_int(option_section, 'ndim_redshifts', default=15),
...
def get_class_inputs(block, config):
    ...
    params = {
        ...
        # 'ln10^{10}A_s':       np.log(1e10*block[cosmo, 'A_s']),
        'ln10^{10}A_s':       block[cosmo, 'logA'],
        'n_s':       block[cosmo, 'n_s'],
        ...
        'N_ur':      nnu - nmassive,
        'N_ncdm':    nmassive,
        'ndim_redshifts': config['ndim_redshifts'], # number of z's where pk emulators are called.
        'skip_background_and_thermo': 0, # background required.
        'skip_pkl': 0 # linear pk required.
    }
    ...
    if config["mpk"]:
        params.update({
            ...
            'z_pk': ', '.join(str(z) for z in np.arange(0.0, config['zmax'], 0.04)),
...
def get_class_outputs(block, c, config):
    ...
    if 'mPk' in c.pars['output']:
        ...
        print('sigma8:',c.sigma8())

        # Total matter power spectrum (saved as grid)
        if config['save_matter_power_lin']:
            # print('[class_sz] For now, we dont save save_matter_power_lin with class_sz')
            ...
            for j, zi in enumerate(z):
                P[:, j] = np.vectorize(c.pk_lin)(k, zi)
            print("Pklin:",P[:2,:2])
        ...
        if config['save_cdm_baryon_power_lin']:
            print('[class_sz] For now, we dont save save_cdm_baryon_power_lin with class_sz')
            # P = np.zeros((k.size, z.size))
            # for i, ki in enumerate(k):
            #     for j, zi in enumerate(z):
        ...
        D = np.vectorize(c.scale_independent_growth_factor)(z)
        print('D:',D[:5])
        # f = [c.scale_independent_growth_factor_f(zi) for zi in z]
        f = np.vectorize(c.scale_independent_growth_factor_f)(z)
        print('f:',f[:5])
        # fsigma = [c.effective_f_sigma8(zi) for zi in z]
        fsigma = np.vectorize(c.effective_f_sigma8)(z)
        print('fsigma8:',fsigma[:5])
        # sigma_8_z = [c.sigma(8.0, zi, h_units=True) for zi in z]
        sigma_8_z = np.vectorize(c.sigma)(8.0, z, h_units=True)
        print('sigma8:',sigma_8_z[:5])
        ...
        if c.nonlinear_method != 0:
            ...
            #         P[i, j] = c.pk(ki, zi)
            for j, zi in enumerate(z):
                P[:, j] = np.vectorize(c.pk)(k, zi)
            print("Pknonlin:",P[:2,:2])
    ...
    d_a = np.array(np.vectorize(c.angular_distance)(z))
    ...
    h_z = np.array(np.vectorize(c.Hubble)(z))
...
def execute(block, config):
    ...
    try:
        ...
        c.set(params)
        print('running class_sz fast')
        print('for params:',params)
        # Run calculations
        if config['use_class_sz_fast_mode']:
            c.compute_class_szfast()
        else:
            c.compute()
        print('done running class_sz fast')

        # Extract outputs
        get_class_outputs(block, c, config)
    except classy_sz.CosmoError as error:
        if config['debug']:
